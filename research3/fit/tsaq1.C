...
for (int i = 0; i <= nScan; i++) {
	...
	//cout << "yfit = " << yfit;
        ...
        //double ey_stat = gr->GetErrorY(j);
        //cout << "ey_stat = " << ey_stat;
        // 如果统计误差不是存在 gr->GetErrorY(j)，
        // 改成：
        // double ey_stat = yStat[j];
	...
	gChi2Nor->SetPoint(ip, Norval, chi2st);
    	ip++;
...
TCanvas *cChi2 = new TCanvas("cChi2", "chi2 vs Nor", 800, 600);
