...
const double kNorMin = 0.001;
const double kNorMax = 0.20;
const double kQMin = 1.06;
const double kQMax = 1.16;
const double kTMin = 0.12;
const double kTMax = 0.40;
...
void tsallis_all_scans()
{
    ...
    TFile *file = TFile::Open("HEPData-ins1394677-v1-Table_3.root");
    if (!file || file->IsZombie()) {
        cerr << "Failed to open HEPData-ins1394677-v1-Table_3.root" << endl;
        return;
    }

    TDirectoryFile *dir = (TDirectoryFile*)file->Get("Table 3");
    if (!dir) {
        cerr << "Failed to find directory: Table 3" << endl;
        return;
    }
