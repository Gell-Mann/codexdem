...
const double kNorMin = 100.0;
const double kNorMax = 5000.0;
const double kQMin = 1.12;
const double kQMax = 1.20;
const double kTMin = 0.02;
const double kTMax = 0.12;
...
void tsallis_all_scans()
{
    ...
    TFile *file = TFile::Open("HEPData-ins1394677-v1-Table_1.root");
    if (!file || file->IsZombie()) {
        cerr << "Failed to open HEPData-ins1394677-v1-Table_1.root" << endl;
        return;
    }

    TDirectoryFile *dir = (TDirectoryFile*)file->Get("Table 1");
    if (!dir) {
        cerr << "Failed to find directory: Table 1" << endl;
        return;
    }
