...
const double kNorMin = 1.0;
const double kNorMax = 10000.0;
const double kQMin = 1.13;
const double kQMax = 1.22;
const double kTMin = 0.015;
const double kTMax = 0.09;
                              ...
                              TH1F *eStat)
{
    ...
    TF1 *f = new TF1(("f_minos_" + result.name).c_str(), TsallisFuncAll, 0.0, 14.0, 3);
    ConfigureFunction(f);
                          ...
                          int nStatPoints)
{
    ...
    TF1 *f = new TF1(("f_" + name).c_str(), TsallisFuncAll, 0.0, 14.0, 3);
    ConfigureFunction(f);
                     ...
                     const char *fileName)
{
    ...
    TF1 *f = new TF1(("f_fit_" + title).c_str(), TsallisFuncAll, 0.0, 14.0, 3);
    ConfigureFunction(f);
                                       ...
                                       int nY)
{
    ...
    TF1 *f = new TF1(("f_pair_" + name).c_str(), TsallisFuncAll, 0.0, 14.0, 3);
    ConfigureFunction(f);
                                 ...
                                 int nStatPoints)
{
    ...
    TF1 *f = new TF1("f_qT_contour_all", TsallisFuncAll, 0.0, 14.0, 3);
    ConfigureFunction(f);
...
void tsallis_all_scans()
{
    ...
    TFile *file = TFile::Open("HEPData-ins1394677-v1-Table_2.root");
    if (!file || file->IsZombie()) {
        cerr << "Failed to open HEPData-ins1394677-v1-Table_2.root" << endl;
        return;
    }

    TDirectoryFile *dir = (TDirectoryFile*)file->Get("Table 2");
    if (!dir) {
        cerr << "Failed to find directory: Table 2" << endl;
        return;
    }

    TGraphAsymmErrors *gr = (TGraphAsymmErrors*)dir->Get("Graph1D_y1");
    TH1F *eStat = (TH1F*)dir->Get("Hist1D_y1_e1");
    TH1F *eSys = (TH1F*)dir->Get("Hist1D_y1_e2");

    if (!gr || !eStat || !eSys) {
        cerr << "Failed to load Graph1D_y1, Hist1D_y1_e1, or Hist1D_y1_e2" << endl;
        return;
    }
    ...
    cout << "Input: Table 2 / Graph1D_y1" << endl;
    cout << "chi2: statistical errors from Hist1D_y1_e1" << endl;
    cout << "N(stat points) = " << nStatPoints << endl;

    resultFile << "Input: Table 2 / Graph1D_y1\n";
    resultFile << "chi2: statistical errors from Hist1D_y1_e1\n";
