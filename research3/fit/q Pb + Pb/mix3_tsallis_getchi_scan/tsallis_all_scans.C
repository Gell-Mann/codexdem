...
const double kNorMin = 10.0;
const double kNorMax = 100000.0;
const double kQMin = 1.05;
const double kQMax = 1.16;
const double kTMin = 0.005;
...
const string kOutputDir = "outputs";
const string kInputFile = "mix3.root";
const string kGraphName = "Graph1D_y3";
...
void ConfigureFunction(TF1 *f)
{
    ...
    f->SetParLimits(1, 1.001, 1.25);
    f->SetParLimits(2, 0.001, 5.0);
}
...
TH1F *BuildStatErrorsFromGraph(TGraphAsymmErrors *gr)
{
    if (!gr) {
        return nullptr;
    }

    TH1F *eStat = new TH1F("stat_errors_from_graph", "stat_errors_from_graph",
                           gr->GetN(), 0.0, gr->GetN());
    eStat->SetDirectory(nullptr);
    ...
    for (int i = 0; i < gr->GetN(); ++i) {
        ...
        const double eyLow = gr->GetErrorYlow(i);
        const double eyHigh = gr->GetErrorYhigh(i);
        double ey = std::max(eyLow, eyHigh);
        if (eyLow > 0.0 && eyHigh > 0.0) {
            ey = 0.5 * (eyLow + eyHigh);
        }

        eStat->SetBinContent(i + 1, y);
        eStat->SetBinError(i + 1, ey);
    }
                              ...
                              TH1F *eStat)
{
    ...
    result.bestChi2 = f->GetChisquare();
...
void PrintScanResult(ostream &out, const ScanResult &result)
{
    ...
    out << "chi2_ROOT = " << FormatDouble(result.bestChi2) << "\n";
    out << "ndf = " << result.ndf << "\n";
    out << "chi2_ROOT/ndf = " << FormatDouble(result.bestChi2 / result.ndf) << "\n";
...
void SaveProfilePlot(const ScanResult &result, const char *fileName)
{
    ...
    g->SetTitle(("#chi^{2}_{ROOT} vs " + result.parameterName + ";" +
                 result.parameterName + ";#chi^{2}_{ROOT}")
                    .c_str());
    g->SetMarkerStyle(20);
    g->SetMarkerSize(0.6);
    g->SetLineWidth(2);
    ...
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->AddEntry(g, "#chi^{2}_{ROOT}", "lp");
                                       ...
                                       int nY)
{
    ...
    hDelta->SetTitle(Form("#Delta#chi^{2}_{ROOT}(%s,%s);%s;%s",
                          result.xName.c_str(), result.yName.c_str(),
                          result.xName.c_str(), result.yName.c_str()));
                                 ...
                                 int nStatPoints)
{
    ...
    hDelta->SetTitle("#Delta#chi^{2}_{ROOT}(q,T);q;T");
...
void PrintPairProfileResult(ostream &out, const PairProfileResult &result)
{
    ...
    out << "errors: Delta chi2_ROOT = " << FormatDouble(result.errorDeltaChi2)
...
void tsallis_all_scans()
{
    gErrorIgnoreLevel = kFatal;
    gSystem->mkdir(kOutputDir.c_str(), true);

    TFile *file = TFile::Open(kInputFile.c_str());
    if (!file || file->IsZombie()) {
        cerr << "Failed to open " << kInputFile << endl;
        return;
    }

    TGraphAsymmErrors *gr = (TGraphAsymmErrors*)file->Get(kGraphName.c_str());
    TH1F *eStat = BuildStatErrorsFromGraph(gr);

    if (!gr || !eStat) {
        cerr << "Failed to load " << kGraphName
             << " or build y-error histogram from it" << endl;
        return;
    }
    ...
    cout << "Input: " << kInputFile << " / " << kGraphName << endl;
    cout << "chi2: ROOT TF1::GetChisquare() after fitting y errors from "
         << kGraphName << endl;
    cout << "N(stat points) = " << nStatPoints << endl;

    resultFile << "Input: " << kInputFile << " / " << kGraphName << "\n";
    resultFile << "chi2: ROOT TF1::GetChisquare() after fitting y errors from "
               << kGraphName << "\n";
