...
string gOutputDir = "tsallis_graph_outputs";

string OutputPath(const string &fileName)
{
    return gOutputDir + "/" + fileName;
}
                                 ...
                                 int nStatPoints)
{
    ...
    TH2D *hChi2 = new TH2D("hChi2_qT_all",
                           ...
                           kNQContour, gQMin, gQMax,
                           kNTContour, gTMin, gTMax);

    double initNor = 10.0;

    for (int iq = 0; iq < kNQContour; ++iq) {
        const double qValue = gQMin + (gQMax - gQMin) * iq / (kNQContour - 1);

        for (int iT = 0; iT < kNTContour; ++iT) {
            const double TValue = gTMin + (gTMax - gTMin) * iT / (kNTContour - 1);
...
void ConfigureJob(const FitJob &job)
{
    gOutputDir = job.outputDir;
    gNorMin = job.norMin;
    gNorMax = job.norMax;
    gQMin = 1.08;
    gQMax = 1.14;
...
void PrintSavedOutputs()
{
    cout << "\nSaved output directory: " << gOutputDir << endl;
...
void RunOneTsallisScan(const FitJob &job)
{
    ConfigureJob(job);
    ...
    const string graphName = Form("Graph1D_y%d", job.graphIndex);
    const string statName = Form("Hist1D_y%d_e1", job.graphIndex);
    const string sysName = Form("Hist1D_y%d_e2", job.graphIndex);

    TGraphAsymmErrors *gr = (TGraphAsymmErrors*)dir->Get(graphName.c_str());
    TH1F *eStat = (TH1F*)dir->Get(statName.c_str());
    TH1F *eSys = (TH1F*)dir->Get(sysName.c_str());

    if (!gr || !eStat || !eSys) {
        cerr << "Failed to load " << graphName << ", "
             << statName << ", or " << sysName << endl;
        return;
    }
    ...
    cout << "\nInput: Figure 15 / " << graphName << endl;
    cout << "chi2: statistical errors from " << statName << endl;
    cout << "N(stat points) = " << nStatPoints << endl;
    cout << "scan ranges: Nor [" << gNorMin << ", " << gNorMax
         << "], q [" << gQMin << ", " << gQMax
         << "], T [" << gTMin << ", " << gTMax << "]" << endl;

    resultFile << "Input: Figure 15 / " << graphName << "\n";
    resultFile << "chi2: statistical errors from " << statName << "\n";
    resultFile << "N(stat points) = " << nStatPoints << "\n";
    resultFile << "scan ranges: Nor [" << gNorMin << ", " << gNorMax
               << "], q [" << gQMin << ", " << gQMax
               << "], T [" << gTMin << ", " << gTMax << "]\n";
    ...
    gSystem->CopyFile("HEPData-ins2061074-v1-Figure_15.root",
                      OutputPath("HEPData-ins2061074-v1-Figure_15.root").c_str(),
                      true);
    gSystem->CopyFile("tsallis_y234_scans.C",
                      OutputPath("tsallis_y234_scans.C").c_str(),
                      true);

    PrintSavedOutputs();
}

void tsallis_y234_scans()
{
    gErrorIgnoreLevel = kFatal;

    vector<FitJob> jobs;
    jobs.push_back({2, "tsallis_graph1d_y2_outputs", 0.30, 40.0});
    jobs.push_back({3, "tsallis_graph1d_y3_outputs", 0.20, 35.0});
    jobs.push_back({4, "tsallis_graph1d_y4_outputs", 0.03, 6.0});

    for (int i = 0; i < (int)jobs.size(); ++i) {
        RunOneTsallisScan(jobs[i]);
    }
}
