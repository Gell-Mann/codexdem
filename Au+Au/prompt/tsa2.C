...
void tsa2()
{
	...
	constexpr const char *statErrorName = "Hist1D_y2_e1";
	constexpr const char *sysErrorName = "Hist1D_y2_e2";
	...
	auto *gr = dynamic_cast<TGraphAsymmErrors*>(inputGraph->Clone("directPhotonSpectrum_y2"));
	auto *e1 = dynamic_cast<TH1F*>(inputE1->Clone("directPhotonSpectrum_y2_stat"));
	auto *e2 = dynamic_cast<TH1F*>(inputE2->Clone("directPhotonSpectrum_y2_sys"));
	if (!gr || !e1 || !e2) {
		gErrorIgnoreLevel = oldErrorIgnoreLevel;
	...
	f1->SetParameter(0, 6.55588);
	f1->SetParLimits(0, 3.36271, 12.);
     	f1->FixParameter(1, 1.117);
	f1->SetParameter(2, 0.078);
