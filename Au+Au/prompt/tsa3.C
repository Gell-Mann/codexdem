...
void tsa3()
{
	...
	constexpr const char *statErrorName = "Hist1D_y3_e1";
	constexpr const char *sysErrorName = "Hist1D_y3_e2";
	...
	auto *gr = dynamic_cast<TGraphAsymmErrors*>(inputGraph->Clone("directPhotonSpectrum_y3"));
	auto *e1 = dynamic_cast<TH1F*>(inputE1->Clone("directPhotonSpectrum_y3_stat"));
	auto *e2 = dynamic_cast<TH1F*>(inputE2->Clone("directPhotonSpectrum_y3_sys"));
	if (!gr || !e1 || !e2) {
		gErrorIgnoreLevel = oldErrorIgnoreLevel;
	...
	f1->SetParameter(0, 2.13295);
	f1->SetParLimits(0, 1.34511, 9.);
     	f1->FixParameter(1, 1.118);
	f1->SetParameter(2, 0.059);
