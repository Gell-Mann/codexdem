                                             ...
                                             double norMax = std::numeric_limits<double>::infinity())
{
    ...
    auto hist = std::make_unique<TH2D>(Form("delta_nc_%s", dataSet.label),
...
void qt_contours_noconstraint()
{
    ...
    // 四档完全独立拟合：不再施加跨中心度的 Nor 单调约束（norMax = +inf）。
    // 因此每一档的 Nor 都由自身数据自由决定，不受相邻档影响。
    std::vector<std::unique_ptr<TH2D>> contourHists;
    std::vector<ScanResult> scanResults;
    for (auto &dataSet : dataSets) {
        ScanResult result;
        const double norMax = std::numeric_limits<double>::infinity();
    ...
    // 绘制 Delta chi2 轮廓。
    gStyle->SetOptStat(0);
    auto *canvas = new TCanvas("c_qt_contours_nc", "q-T contours (no constraint)", 900, 700);
    auto *frame = new TH2D("frame_nc", "", 10, 1.0, 1.2, 10, 0.0, 0.30);
    ...
    canvas->SaveAs("qt_contours_noconstraint_figure16_0_10.png");
    canvas->SaveAs("qt_contours_noconstraint_figure16_0_10.pdf");
}
