...
void tsa2()
{
	...
	constexpr const char *graphName = "Graph1D_y2";
	constexpr const char *statErrorName = "Hist1D_y2_e1plus";
	constexpr const char *sysErrorName = "Hist1D_y2_e2plus";
	...
	auto *gr = dynamic_cast<TGraphAsymmErrors*>(inputGraph->Clone("directPhotonSpectrum_y1"));
	auto *e1 = dynamic_cast<TH1F*>(inputE1->Clone("directPhotonSpectrum_y1_stat"));
	auto *e2 = dynamic_cast<TH1F*>(inputE2->Clone("directPhotonSpectrum_y1_sys"));
	if (!gr || !e1 || !e2) {
		gErrorIgnoreLevel = oldErrorIgnoreLevel;
	...
	f1->SetParameter(0, 3.);
	f1->SetParLimits(0, 2.06, 6.);
     	f1->FixParameter(1, 1.09);
	f1->SetParLimits(1, 1.091, 1.13);
	f1->SetParameter(2, 0.107);
	f1->SetParLimits(2, 0.001, 0.18);
